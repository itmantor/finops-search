...
## جستجوی گروهی از فایل اکسل

صفحه‌ی وب علاوه بر جستجوی تکی، آپلود یک فایل xlsx/csv (حداکثر ۲۰۰ ردیف) را
هم پشتیبانی می‌کند — دکمه‌ی «دانلود قالب اکسل» ساختار دقیق ستون‌ها را می‌دهد.
پردازش در پس‌زمینه انجام می‌شود (`pipeline/bulk_lookup.py` به‌صورت یک
پردازه‌ی جدا، نه ترد داخل gunicorn) تا نه با timeout پراکسی ۱۲۰ ثانیه‌ی nginx
تداخل کند و نه با ری‌استارت وب‌سرور از بین برود؛ صفحه با polling پیشرفت را
نشان می‌دهد و کاربر می‌تواند تب را ببندد و بعداً برگردد.

فایل‌های آپلودی و نتیجه در `uploads/bulk/<job_id>/` نگه داشته می‌شوند (هرگز
در گیت)، و هر بار که آپلود تازه‌ای می‌آید، jobهای قدیمی‌تر از
`pipeline/bulk_shared.RETENTION_DAYS` روز خودکار پاک می‌شوند.

آپلود دوباره‌ی فایل نتیجه Resume دارد: ردیف‌های «یافت شد»/«یافت نشد» دوباره
پردازش نمی‌شوند (جستجو قطعی است، تکرارش فقط هزینه‌ی API را هدر می‌دهد)، و
ردیف‌های «نیاز به بررسی» فقط وقتی کاربر ستون «انتخاب شما» را با یکی از
گزینه‌های ستون «گزینه‌های دیگر» پر کرده باشد به‌صورت محلی (بدون فراخوانی API)
نهایی می‌شوند. تشخیص ابهام دقیقاً همان تابع مشترک وب‌سایت را استفاده می‌کند
(`search/ambiguity.py`) تا هر دو مسیر یک قانون را ببینند.
...
├── uploads/bulk/<job_id>/      ← فایل ورودی/نتیجه‌ی هر job جستجوی گروهی (در گیت نیست، چند روزه پاک می‌شود)
├── common/
...
│   ├── ambiguity.py              ← تشخیص ابهام دسته‌بندی (مشترک بین وب و جستجوی گروهی)
│   ├── engine.py                 ← راه‌اندازی مشترک کل پشته‌ی جستجو (مشترک بین وب و جستجوی گروهی)
│   ├── taxonomy_labels.py        ← برچسب کوتاه فارسی هر level1 برای چیپ‌ها
...
│   ├── bulk_shared.py           ← ستون‌ها/خواندن‌ونوشتن فایل جستجوی گروهی
│   ├── bulk_template.py         ← ساخت فایل قالب اکسل قابل‌دانلود
│   ├── bulk_lookup.py           ← پردازش پس‌زمینه‌ی جستجوی گروهی (به‌صورت پردازه‌ی جدا اجرا می‌شود)
